...
For an authorized scout, the Agent may preregister and choose learning rate, one trusted primary loss, one trusted augmentation policy, architecture, input mode, sampling traversal policy, optimizer, auxiliary targets, and other allowlisted Candidate fields. A valid preregistered choice is not a protocol deviation merely because it differs from a prior Candidate. Candidate source still cannot implement or redefine provider-owned loss, augmentation, data, sampling, metric, Artifact Filter, resource, or lifecycle behavior, and the generated Workspace ceilings remain authoritative.
