"""Randomly initialized MCAST 1.1-lineage positive-control architecture."""
...
class MCAST11PositiveControl(nn.Module):
    """Derive MCAST 1.1 planes and emit one contrail-mask logit plane."""
...
def build_model(input_spec: dict[str, object], output_spec: dict[str, object]) -> nn.Module:
    """Build the positive control for the Harness-supplied ABI contract."""

    if input_spec.get("mode") != "abi_16ch" or input_spec.get("shape") != [16, 256, 256]:
        raise ValueError("abi031_mcast11_positive_control_v1 requires abi_16ch [16, 256, 256] input")
    if output_spec.get("form") != "mask_logits" or output_spec.get("shape") != [1, 256, 256]:
        raise ValueError("abi031_mcast11_positive_control_v1 requires mask_logits [1, 256, 256] output")
    return MCAST11PositiveControl()
