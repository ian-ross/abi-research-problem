"""MCAST 1.1-lineage model for a controlled focal-Tversky continuation."""
...
class MCAST11FocalTverskyModel(nn.Module):
...
def build_model(input_spec: dict[str, object], output_spec: dict[str, object]) -> nn.Module:
    ...
    if input_spec.get("mode") != "abi_16ch" or input_spec.get("shape") != [16, 256, 256]:
        raise ValueError("abi032_mcast11_focal_tversky_v1 requires abi_16ch [16, 256, 256] input")
    if output_spec.get("form") != "mask_logits" or output_spec.get("shape") != [1, 256, 256]:
        raise ValueError("abi032_mcast11_focal_tversky_v1 requires mask_logits [1, 256, 256] output")
    return MCAST11FocalTverskyModel()
